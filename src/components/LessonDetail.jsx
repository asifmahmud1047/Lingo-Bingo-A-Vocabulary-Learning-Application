    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      {vocabularies.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {vocabularies.map((word) => (
            <div
              key={word.id}
              className={`rounded-lg border-2 p-3 sm:p-4 shadow-md ${getDifficultyColor(
                word.difficulty
              )}`}
            >
              <div className="flex justify-between items-start mb-2">
                <h2 className="text-xl sm:text-2xl font-bold cursor-pointer" onClick={() => speakWord(word.word)}>
                  {word.word}
                </h2>
                <span className="text-xs sm:text-sm bg-gray-200 rounded-full px-2 py-1">
                  {word.part_of_speech}
                </span>
              </div>
      ...
      {isModalOpen && selectedWord && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg max-w-2xl w-full">
            <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">{selectedWord.word}</h2>
            ...
            <div className="bg-gray-100 p-2 sm:p-3 rounded text-sm sm:text-base">
              <p className="font-semibold">Example:</p>
              <p>{selectedWord.example}</p>
            </div>
            <button
              onClick={closeModal}
              className="mt-4 sm:mt-6 bg-red-500 hover:bg-red-600 text-white py-1 sm:py-2 px-3 sm:px-4 rounded"
            >
              Close
            </button>
          </div>
